...
def compute_weather_metrics(pred, target, threshold=0.1):
    """Compute CSI, FAR, POD, HSS"""
...
def compute_pooled_csi(pred, target, pool_size, threshold=0.1):
    """Compute CSI with spatial pooling"""
    if len(pred.shape) == 5:  # b t h w c
        pred = pred.permute(0, 1, 4, 2, 3)  # b t c h w
        target = target.permute(0, 1, 4, 2, 3)
        pred = pred.reshape(-1, pred.shape[2], pred.shape[3], pred.shape[4])  # (bt) c h w
        target = target.reshape(-1, target.shape[2], target.shape[3], target.shape[4])
    
    pred_pooled = F.avg_pool2d(pred, kernel_size=pool_size, stride=pool_size)
    target_pooled = F.avg_pool2d(target, kernel_size=pool_size, stride=pool_size)
    
    pred_binary = (pred_pooled > threshold).float()
    target_binary = (target_pooled > threshold).float()
...
def compute_crps(pred, target):
    """Compute CRPS (for deterministic predictions, CRPS = MAE)"""
    crps = F.l1_loss(pred, target)
    return crps.item()
...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    def test_step(self, batch, batch_idx):
        ...
        if not self.eval_example_only or data_idx in self.val_example_data_idx_list:
            ...
            if not hasattr(self, 'test_aligned_preds'):
                self.test_aligned_preds = []
                self.test_aligned_targets = []
                self.test_preds = []
                self.test_targets = []
            ...
            for i in range(self.oc.eval.num_samples_per_context):
                ...
                if self.use_alignment and self.oc.eval.eval_aligned:
                    ...
                    self.test_aligned_preds.append(pred_seq.detach())
                    self.test_aligned_targets.append(target_seq.detach())
                ...
                if self.oc.eval.eval_unaligned:
                    ...
                    # Store for epoch-end computation
                    self.test_preds.append(pred_seq.detach())
                    self.test_targets.append(target_seq.detach())
    ...
    def on_test_epoch_end(self):
        ...
        if self.oc.eval.eval_unaligned and hasattr(self, 'test_preds') and len(self.test_preds) > 0:
            all_preds = torch.cat(self.test_preds, dim=0)
            all_targets = torch.cat(self.test_targets, dim=0)
            
            # Weather metrics
            weather_metrics = compute_weather_metrics(all_preds, all_targets)
            self.log("test_csi", weather_metrics['csi'], sync_dist=True)
            self.log("test_far", weather_metrics['far'], sync_dist=True)
            self.log("test_pod", weather_metrics['pod'], sync_dist=True)
            self.log("test_hss", weather_metrics['hss'], sync_dist=True)
            
            # Pooled CSI
            csi_4x4 = compute_pooled_csi(all_preds, all_targets, pool_size=4)
            csi_16x16 = compute_pooled_csi(all_preds, all_targets, pool_size=16)
            self.log("test_csi_pool_4x4", csi_4x4, sync_dist=True)
            self.log("test_csi_pool_16x16", csi_16x16, sync_dist=True)
            
            # Regression metrics
            reg_metrics = compute_regression_metrics(all_preds, all_targets)
            self.log("test_mse", reg_metrics['mse'], sync_dist=True)
            self.log("test_mae", reg_metrics['mae'], sync_dist=True)
            self.log("test_rmse", reg_metrics['rmse'], sync_dist=True)
            self.log("test_psnr", reg_metrics['psnr'], sync_dist=True)
            
            # CRPS
            crps = compute_crps(all_preds, all_targets)
            self.log("test_crps", crps, sync_dist=True)
            
            # SSIM and LPIPS (convert to bchw format)
            all_preds_bchw = rearrange(all_preds, "b t h w c -> (b t) c h w")
            all_targets_bchw = rearrange(all_targets, "b t h w c -> (b t) c h w")
            
            ssim = compute_ssim(all_preds_bchw, all_targets_bchw)
            lpips = compute_lpips(all_preds_bchw, all_targets_bchw)
            self.log("test_ssim", ssim, sync_dist=True)
            self.log("test_lpips", lpips, sync_dist=True)
        
        # Compute metrics for aligned predictions
        if (self.use_alignment and self.oc.eval.eval_aligned and 
            hasattr(self, 'test_aligned_preds') and len(self.test_aligned_preds) > 0):
            
            all_aligned_preds = torch.cat(self.test_aligned_preds, dim=0)
            all_aligned_targets = torch.cat(self.test_aligned_targets, dim=0)
            
            # Weather metrics
            weather_metrics = compute_weather_metrics(all_aligned_preds, all_aligned_targets)
            self.log("test_aligned_csi", weather_metrics['csi'], sync_dist=True)
            self.log("test_aligned_far", weather_metrics['far'], sync_dist=True)
            self.log("test_aligned_pod", weather_metrics['pod'], sync_dist=True)
            self.log("test_aligned_hss", weather_metrics['hss'], sync_dist=True)
            
            # Pooled CSI
            csi_4x4 = compute_pooled_csi(all_aligned_preds, all_aligned_targets, pool_size=4)
            csi_16x16 = compute_pooled_csi(all_aligned_preds, all_aligned_targets, pool_size=16)
            self.log("test_aligned_csi_pool_4x4", csi_4x4, sync_dist=True)
            self.log("test_aligned_csi_pool_16x16", csi_16x16, sync_dist=True)
            
            # Regression metrics
            reg_metrics = compute_regression_metrics(all_aligned_preds, all_aligned_targets)
            self.log("test_aligned_mse", reg_metrics['mse'], sync_dist=True)
            self.log("test_aligned_mae", reg_metrics['mae'], sync_dist=True)
            self.log("test_aligned_rmse", reg_metrics['rmse'], sync_dist=True)
            self.log("test_aligned_psnr", reg_metrics['psnr'], sync_dist=True)
            
            # CRPS
            crps = compute_crps(all_aligned_preds, all_aligned_targets)
            self.log("test_aligned_crps", crps, sync_dist=True)
            
            # SSIM and LPIPS
            all_aligned_preds_bchw = rearrange(all_aligned_preds, "b t h w c -> (b t) c h w")
            all_aligned_targets_bchw = rearrange(all_aligned_targets, "b t h w c -> (b t) c h w")
            
            ssim = compute_ssim(all_aligned_preds_bchw, all_aligned_targets_bchw)
            lpips = compute_lpips(all_aligned_preds_bchw, all_aligned_targets_bchw)
            self.log("test_aligned_ssim", ssim, sync_dist=True)
            self.log("test_aligned_lpips", lpips, sync_dist=True)
        
        # Clear stored predictions to free memory
        if hasattr(self, 'test_preds'):
            self.test_preds.clear()
            self.test_targets.clear()
        if hasattr(self, 'test_aligned_preds'):
            self.test_aligned_preds.clear()
            self.test_aligned_targets.clear()
...
def main():
    ...
    dm.prepare_data()
    dm.setup()
    print(dm.start_date, dm.end_date)
    exit()
