...
class RTimeSeriesDataset(Dataset):
    def __init__(self, file_pairs,data_dir,mask, mean = None, std = None):
    ...
    def _load_file(self, filename):
        ...
        if filename not in self.cache:
            ...
            data_array = preprocessing_normalization(self.mask, data_array, self.mean, self.std)
...
# def preprocessing_lstm(ds, mask):
...
#     np_reflec[np_reflec<0]=0
#     np_reflec[np_reflec>45]=0
#     return np.expand_dims(np_reflec, axis=0)

def preprocessing_lstm(mask,ds, mean = None, std = None):
    ...
    if mean is not None and std is not None:
        assert np_reflec.shape == mean.shape, "Shape mismatch between data and mean"
        np_reflec = (np_reflec - mean) / std
    np_reflec[np.isnan(np_reflec)] = 0
    np_reflec = np.clip(np_reflec, 0, 50)

    return np_reflec

def preprocessing_normalization(mask,ds, min = None, max = None):
