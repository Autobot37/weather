...
class SEVIRSkillScore(Metric):
    ...
    def compute(self):
        ...
        # Filter metrics_list to only include those supported by SEVIRSkillScore
        supported_metrics = [m for m in self.metrics_list if m in metrics_dict]
        
        for metrics in supported_metrics:
            if self.keep_seq_len_dim:
