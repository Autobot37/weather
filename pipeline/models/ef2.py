...
class DEarthformer(BaseModel):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Normalize to [0, 1]
        sampled = sampled.clamp(-1, 1).add(1).div(2).detach()
        tgt = tgt.clamp(-1, 1).add(1).div(2).detach()
...
if __name__ == "__main__":
    ...
    logger = WandbLogger(project="DEarthformer2", save_dir="logs/DEarthformer2")
    run_id = logger.version 
    print(f"Logger: {logger.name}, Run ID: {run_id}") 
    
    checkpoint_callback = ModelCheckpoint(
        dirpath=f"logs/DEarthformer2/checkpoints/{run_id}",
        filename="DEarthformer2-{epoch:02d}-{step:06d}",
        save_top_k=-1,
        save_last=True,
