...
################### mainly reference to https://github.com/facebookresearch/DiT ###################
################### mainly reference to https://github.com/facebookresearch/DiT ###################
################### mainly reference to https://github.com/facebookresearch/DiT ###################
...
class _DiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_embedder = PatchEmbed(input_size, patch_size, in_channels, enc_hidden_size, bias=True)
        self.t_embedder = TimestepEmbedder(enc_hidden_size)
        self.aggr_t_embedder = TimestepEmbedder(hidden_size)
        # self.y_embedder = LabelEmbedder(num_classes, hidden_size, class_dropout_prob)
        num_patches = self.x_embedder.num_patches
        # Will use fixed sin-cos embedding:
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, enc_hidden_size), requires_grad=False)

        
        self.encoder_blocks = nn.ModuleList([
            DiTBlock(enc_hidden_size, single_heads_num, mlp_ratio=mlp_ratio) for _ in range(enc_depth)
        ])
        self.enc2hidden_linear = nn.Linear(in_features=enc_hidden_size*split_num, out_features=hidden_size, bias=True)
    ...
    def initialize_weights(self):
        ...
        for block in self.encoder_blocks+self.blocks:
    ...
    def forward(self, x, t):
        ...
        latent_x = self.enc2hidden_linear(latent_x)
        for block in self.blocks:
            latent_x = block(latent_x, aggr_t)

        out = self.final_layer(latent_x, aggr_t)                # (N, T, patch_size ** 2 * out_channels)
        out = self.unpatchify(out)                   # (N, out_channels, H, W)
        out = rearrange(out, 'b (t c) h w -> b t c h w', t=T)  # (N, T, out_channels, H, W)
        return out
