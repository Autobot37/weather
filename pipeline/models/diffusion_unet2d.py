...
class Model(BaseModel):
    def __init__(self, data_config):
        super().__init__(model_name="HFUnet")
        ...
        self.unet = WrappedUNet2D(sample_size = self.image_size, in_channels=in_channels, out_channels=in_channels, num_train_timesteps=1000)

    def forward(self, noisy, timesteps, cond):
        # noisy: (B, T, H, W, C), cond: (B, T, H, W, C), timesteps: (B,)
        return self.unet(noisy, timesteps, cond)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx % 500 == 0:
            ...
            tgt = tgt.detach()
            sampled = sampled.unsqueeze(2)  # (B, T, 1, H, W)
            tgt = tgt.unsqueeze(2) # (B, T, 1, H, W)
...
if __name__ == "__main__":
    ...
    logger = WandbLogger(project="HFUnet", save_dir="logs/HFUnet", name = 'test')
    run_id = logger.version 
    print(f"Logger: {logger.name}, Run ID: {run_id}") 

    checkpoint_callback = ModelCheckpoint(
        dirpath=f"logs/HFUnet/checkpoints/{run_id}",
        filename="HFUnet-{epoch:02d}-{step:06d}",
        save_top_k=-1,
        save_last=True,
