...
class Model(BaseModel):
    def __init__(self, data_config):
        super().__init__(model_name="PureDiff")
...
if __name__ == "__main__":
    ...
    logger = WandbLogger(project="PureDiff", save_dir="logs/PureDiff")
    run_id = logger.version
    print(f"Logger: {logger.name}, Run ID: {run_id}")

    checkpoint_callback = ModelCheckpoint(
        dirpath=f"logs/PureDiff/checkpoints/{run_id}",
        filename="PureDiff-{epoch:02d}-{step:06d}",
        save_top_k=-1,
        save_last=True,
