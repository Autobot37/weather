...
import math
from pipeline.modeldefinitions.ef_utils import CuboidSelfAttentionPatterns, CuboidCrossAttentionPatterns
from pipeline.modeldefinitions.ef_utils import (
...
class TimestepEmbedder(nn.Module):
    """
    scalar timesteps -> vector representations.
    """
    def __init__(self, hidden_size = 1024, frequency_embedding_size=256):
        super().__init__()
...
class AdaLayerNorm(nn.Module):
    def __init__(self, inp_channel, timestep_dim = 1024, eps=1e-5):
        super().__init__()
        self.ln = nn.LayerNorm(inp_channel, eps=eps, elementwise_affine=False)
        self.ada_mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(timestep_dim, 2 * inp_channel)
        )
    ...
    def forward(self, x, c):
        """
        x: The input tensor to normalize. Shape (B, ..., C)
        c: The timestep conditioning vector. Shape (B, cond_dim)
        """
        while len(c.shape) < len(x.shape):
            c = c.unsqueeze(1)
        scale, shift = self.ada_mlp(c).chunk(2, dim=-1)
...
class PositionwiseFFN(nn.Module):
    ...
    def __init__(self,
        ...
        self.ffn_2 = nn.Linear(in_features=hidden_size, out_features=units,
                               bias=True)
        self.adalayer_norm = AdaLayerNorm(inp_channel=units, eps = layer_norm_eps)
        self.reset_parameters()
    ...
    def forward(self, data,  time_embedding):
        """
        ...
        # residual = data
        if self._pre_norm:
            data = self.adalayer_norm(data, time_embedding)
        if self._gated_proj:
        ...
        if not self._pre_norm:
            out = self.adalayer_norm(out, time_embedding)
        return out
...
class PatchMerging3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.reduction = nn.Linear(downsample[0] * downsample[1] * downsample[2] * dim,
                                   out_dim, bias=False)
        self.adalayernorm = AdaLayerNorm(inp_channel = downsample[0] * downsample[1] * downsample[2] * dim)
        self.reset_parameters()

    def reset_parameters(self):
        for m in self.children():
            if m is self.adalayernorm:
                continue
    ...
    def forward(self, x,  time_embedding):
        ...
        x = self.adalayernorm(x, time_embedding)
        x = self.reduction(x)
...
class CuboidSelfAttentionLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.adalayernorm = AdaLayerNorm(inp_channel=dim)
        if self.use_global_vector:
            self.global_vec_adalayernorm =  AdaLayerNorm(inp_channel=global_dim_ratio*dim)
    ...
    def forward(self, x, global_vectors=None,  time_embedding=None):
        assert time_embedding is not None, "time_embedding must be provided for CuboidSelfAttentionLayer."
        x = self.adalayernorm(x, time_embedding)
        ...
        if self.use_global_vector:
            ...
            global_vectors = self.global_vec_adalayernorm(global_vectors, time_embedding)
...
class StackCuboidSelfAttentionBlock(nn.Module):
    ...
    def forward(self, x, global_vectors=None,  time_embedding=None):
        assert time_embedding is not None, "time_embedding must be provided for StackCuboidSelfAttentionBlock."
        if self.use_inter_ffn:
            if self.use_global_vector:
                for idx, (attn, ffn) in enumerate(zip(self.attn_l, self.ffn_l)):
                    x_out, global_vectors_out = attn(x, global_vectors, time_embedding)
                    x = x + x_out
                    global_vectors = global_vectors + global_vectors_out

                    if self.checkpoint_level >= 1 and self.training:
                        x = checkpoint.checkpoint(ffn, x, time_embedding)
                        if self.use_global_vector_ffn:
                            global_vectors = checkpoint.checkpoint(self.global_ffn_l[idx], global_vectors, time_embedding)
                    else:
                        x = ffn(x, time_embedding)
                        if self.use_global_vector_ffn:
                            global_vectors = self.global_ffn_l[idx](global_vectors, time_embedding)
                return x, global_vectors
            else:
                for idx, (attn, ffn) in enumerate(zip(self.attn_l, self.ffn_l)):
                    if self.checkpoint_level >= 2 and self.training:
                        x = x + checkpoint.checkpoint(attn, x, time_embedding)
                    else:
                        x = x + attn(x, None, time_embedding)
                    if self.checkpoint_level >= 1 and self.training:
                        x = checkpoint.checkpoint(ffn, x, time_embedding)
                    else:
                        x = ffn(x, time_embedding)
            ...
            if self.use_global_vector:
                for idx, attn in enumerate(self.attn_l):
                    if self.checkpoint_level >= 2 and self.training:
                        x_out, global_vectors_out = checkpoint.checkpoint(attn, x, global_vectors, time_embedding)
                    else:
                        x_out, global_vectors_out = attn(x, global_vectors, time_embedding)
                    x = x + x_out
                    global_vectors = global_vectors + global_vectors_out
                if self.checkpoint_level >= 1 and self.training:
                    x = checkpoint.checkpoint(self.ffn_l[0], x, time_embedding)
                    if self.use_global_vector_ffn:
                        global_vectors = checkpoint.checkpoint(self.global_ffn_l[0], global_vectors, time_embedding)
                else:
                    x = self.ffn_l[0](x, time_embedding)
                    if self.use_global_vector_ffn:
                        global_vectors = self.global_ffn_l[0](global_vectors, time_embedding)
                return x, global_vectors
            else:
                for idx, attn in enumerate(self.attn_l):
                    if self.checkpoint_level >= 2 and self.training:
                        out = checkpoint.checkpoint(attn, x, time_embedding)
                    else:
                        out = attn(x, None, time_embedding)
                    x = x + out
                if self.checkpoint_level >= 1 and self.training:
                    x = checkpoint.checkpoint(self.ffn_l[0], x, time_embedding)
                else:
...
class CuboidCrossAttentionLayer(nn.Module):
    ...
    def forward(self, x, mem, mem_global_vectors=None, time_embedding=None):
        assert time_embedding is not None, "time_embedding must be provided for CuboidCrossAttentionLayer."
...
class CuboidTransformerEncoder(nn.Module):
    ...
    def forward(self, x, global_vectors=None, time_embedding=None):
        assert time_embedding is not None, "time_embedding must be provided for CuboidTransformerEncoder."
        """
...
class StackCuboidCrossAttentionBlock(nn.Module):
    ...
    def forward(self, x, mem, mem_global_vector=None, time_embedding=None):
        assert time_embedding is not None, "time_embedding must be provided for StackCuboidCrossAttentionBlock."
        """
        ...
        if self.use_inter_ffn:
            for attn, ffn in zip(self.attn_l, self.ffn_l):
                if self.checkpoint_level >= 2 and self.training:
                    x = x + checkpoint.checkpoint(attn, x, mem, mem_global_vector, time_embedding)
                else:
                    x = x + attn(x, mem, mem_global_vector, time_embedding)
                if self.checkpoint_level >= 1 and self.training:
                    x = checkpoint.checkpoint(ffn, x, time_embedding)
                else:
                    x = ffn(x, time_embedding)
...
class CuboidTransformerDecoder(nn.Module):
    ...
    def forward(self, x, mem_l, mem_global_vector_l=None, time_embedding=None):
        assert time_embedding is not None, "time_embedding must be provided for CuboidTransformerDecoder."
        """
        ...
        for i in range(self.num_blocks - 1, -1, -1):
            ...
            if not self.use_first_self_attn and i == self.num_blocks - 1:
                ...
                if i >= self.cross_start:
                    x = self.cross_blocks[i - self.cross_start][0](x, mem_l[i], mem_global_vector, time_embedding)
                for idx in range(self.depth[i] - 1):
                    if self.use_self_global:  # in this case `mem_global_vector` is guaranteed to be not None
                        if self.self_update_global:
                            x, mem_global_vector = self.self_blocks[i][idx](x, mem_global_vector, time_embedding)
                        else:
                            x, _ = self.self_blocks[i][idx](x, mem_global_vector, time_embedding)
                    else:
                        x = self.self_blocks[i][idx](x, None, time_embedding)
                    if i >= self.cross_start:
                        x = self.cross_blocks[i - self.cross_start][idx + 1](x, mem_l[i], mem_global_vector, time_embedding)
            else:
                for idx in range(self.depth[i]):
                    if self.use_self_global:
                        ...
                        x = self.self_blocks[i][idx](x, None, time_embedding)
                    if i >= self.cross_start:
                        x = self.cross_blocks[i - self.cross_start][idx](x, mem_l[i], mem_global_vector, time_embedding)
            # Upsample
            if i > 0:
...
class InitialEncoder(nn.Module):
    ...
    def forward(self, x, time_embedding):
        ...
        if self.num_conv_layers > 0:
            ...
            x = self.patch_merge(x.reshape(B, T, H, W, -1), time_embedding)
        else:
            x = self.patch_merge(x, time_embedding)
        return x
...
class InitialStackPatchMergingEncoder(nn.Module):
    ...
    def forward(self, x,  time_embedding):
        ...
        for i, (conv_block, patch_merge) in \
            ...
            x = patch_merge(x, time_embedding)
        return x
...
class CuboidTransformerModel(nn.Module):
    ...
    def forward(self, x, timesteps, verbose=False):
        """
        ...
        x = self.initial_encoder(x, time_embedding)
        ...
        if self.num_global_vectors > 0:
            ...
            mem_l, mem_global_vector_l = self.encoder(x, init_global_vectors, time_embedding)
        else:
            mem_l = self.encoder(x, None, time_embedding)
        if verbose:
        ...
        initial_z = self.get_initial_z(final_mem=mem_l[-1],
                                       T_out=T_out)
        if self.num_global_vectors > 0:
            dec_out = self.decoder(initial_z, mem_l, mem_global_vector_l, time_embedding)
        else:
            dec_out = self.decoder(initial_z, mem_l, None, time_embedding)
        dec_out = self.final_decoder(dec_out)
        out = self.dec_final_proj(dec_out)
        return out
    
# B, T, H, W, C = 2, 10, 64, 64, 1

# model = CuboidTransformerModel(input_shape=(T, H, W, C), target_shape=(T, H, W, C), enc_depth=[2, 2], base_units=32).to("cuda")
# x = torch.randn((B, T, H, W, C)).to("cuda")
# timesteps = torch.randint(0, 1000, (B,)).to("cuda")
# out = model(x, timesteps, verbose=True)
