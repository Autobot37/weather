...
class SEVIRTorchDataset(TorchDataset):
    ...
    def __init__(self,
                 ...
                 preprocess: bool = True,
                 rescale_method: str = "01",
                 downsample_dict: Dict[str, Sequence[int]] = None, # Added downsample_dict
                 verbose: bool = False):
        ...
        self.sevir_dataloader = SEVIRDataLoader(
            ...
            downsample_dict=downsample_dict, # Pass downsample_dict
            verbose=verbose)
...
class SEVIRLightningDataModule(LightningDataModule):
    ...
    def __init__(self):
        ...
        for k, v in cfg.items():
            setattr(self, k, v)
        # Explicitly get downsample_dict from the global cfg
        self.downsample_dict = cfg.get("downsample_dict", None)
        self.output_type = np.float32
        dataset_name = cfg.dataset_name
        start_date = cfg.start_date
        train_val_split_date = cfg.train_val_split_date
        train_test_split_date = cfg.train_test_split_date
    ...
    def setup(self, stage = None) -> None:
        ...
        self.sevir_predict = SEVIRTorchDataset(
            ...
            layout=self.layout,
            num_shard=1, rank=0,
            start_date=self.train_test_split_date, # Or specific predict range
            end_date=self.end_date, # Or specific predict range
            output_type=self.output_type,
            preprocess=self.preprocess,
            rescale_method=self.rescale_method,
            downsample_dict=self.downsample_dict, # Pass downsample_dict
...
if __name__ == "__main__":
    ...
    for loader in [train_loader, val_loader, test_loader]:
        ...
        print(f"end date: {dm.end_date}")
        print(f"train test split date: {dm.train_test_split_date}")
        print(f"train val split date: {dm.train_val_split_date}")

        print(f"seq len: {dm.seq_len}, raw_seq_len: {dm.raw_seq_len}")
        for batch in loader:
            data = batch['vil']
            print(f"Data shape: {data.shape}, dtype: {data.dtype}")
            print(f"Min value: {data.min()}, Max value: {data.max()}")
            max_value = [data.max().item() for data in batch.values()]
            min_value = [data.min().item() for data in batch.values()]
