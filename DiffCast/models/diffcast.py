...
class GaussianDiffusion(nn.Module):
    ...
    # q means forward diffusion process, p means reverse diffusion process
    def _predict(self, frames_in, frames_gt, loss_weight=0.5):
        ...
        pre_mu = None
        diff_loss = 0.0
        t = torch.randint(0, self.num_timesteps, (frames_in.shape[0],), device=frames_in.device, dtype=torch.long)
        T_in = frames_in.shape[1]
        ...
        for frag_idx in range(T_out // T_in):
            ...
            # print("pre frag shape:", pre_frag.shape)
            # print("pre mu shape:", pre_mu.shape if pre_mu is not None else "None") 
            cond = pre_frag - pre_mu if pre_mu is not None else torch.zeros_like(pre_frag)
    ...
    def p_losses(self, x_start, t, noise=None, cond=None, ctx=None, idx=None):        
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        loss = loss * extract(self.loss_weight, t, x_t.shape)
