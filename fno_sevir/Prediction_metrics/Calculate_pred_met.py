...
def cal_batch_avg_rmse(y, y_pred, seq_length):
    bs = y.shape[0]
    y = y.cpu().numpy()
    y_pred = y_pred.cpu().numpy()
    rmse_s = []
    for i in range(bs):
        rmse_seq = []
        for j in range(seq_length):
            rmse_seq.append(RMSE(y[i][j], y_pred[i][j]))
        rmse_seq = np.array(rmse_seq)
        # print(pcc_seq.shape)
        mean = np.mean(rmse_seq, axis = 0)
        rmse_s.append(mean)
        # print(mean.shape)
    
    rmse_s = np.array(rmse_s)
    # print(f"Pccs shape : {pccs}")
    avg_rmse = np.mean(rmse_s, axis = 0)
    # print(f"Avg Pccs shape : {avg_pcc}")
    return avg_rmse
